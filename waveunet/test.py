...
import BSS.waveunet.data.utils
    ...
    with torch.no_grad():
        for target_start_pos in range(0, target_outputs, model.shapes["output_frames"]):
            ...
            for key, curr_targets in compute_model_output(model, curr_input).items():
                outputs[key][:,
                target_start_pos:target_start_pos + model.shapes["output_frames"]] = curr_targets.squeeze(
                    0).cpu().numpy()
...
def predict_song(args, audio_path, model):
    ...
    mix_audio, mix_sr = BSS.waveunet.data.utils.load(audio_path, sr=None, mono=False)
